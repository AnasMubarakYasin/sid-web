import fs from"fs";const wasmPath="./spice.wasm";const gluePath="./spice-patched.js";const adapterPath="./phet-spice.js";const outputPath="./phet-spice-bundle.js";console.log("Reading files...");const wasmBinary=fs.readFileSync(wasmPath);const glueCode=fs.readFileSync(gluePath,"utf8");const adapterCode=fs.readFileSync(adapterPath,"utf8");console.log(`WASM size: ${(wasmBinary.length/1024/1024).toFixed(2)} MB`);console.log(`Glue code: ${(glueCode.length/1024).toFixed(1)} KB`);console.log(`Adapter: ${(adapterCode.length/1024).toFixed(1)} KB`);const wasmBase64=wasmBinary.toString("base64");console.log(`WASM base64: ${(wasmBase64.length/1024/1024).toFixed(2)} MB`);let modifiedGlue=glueCode;modifiedGlue=modifiedGlue.replace(/function findWasmBinary\(\)\{if\(Module\["locateFile"\]\)\{return locateFile\("spice\.wasm"\)\}return new URL\("spice\.wasm",import\.meta\.url\)\.href\}/,'function findWasmBinary(){return Module["locateFile"]("spice.wasm","")}');const bundle=`// Copyright 2025, University of Colorado Boulder
...
${modifiedGlue.replace("export default Module",`
...
${adapterCode.replace("import Module from './spice-patched.js';","// Module imported from above").replace("export class Simulation","class Simulation").replace(/\/\/ Global export[\s\S]*$/,"// Global export handled below")}
...
`;fs.writeFileSync(outputPath,bundle,"utf8");const stats=fs.statSync(outputPath);console.log(`
Bundle created: ${outputPath}`);console.log(`Total size: ${(stats.size/1024/1024).toFixed(2)} MB`);