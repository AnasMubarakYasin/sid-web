 ...
 */module.exports={create:function(context){return{CallExpression:node=>{if(node.callee&&node.callee.type==="MemberExpression"){if(node.callee.property.name==="register"){const registerArgs=node.arguments;if(registerArgs.length===2){const firstArg=registerArgs[0];const secondArg=registerArgs[1];if(firstArg.type==="Literal"&&secondArg.type==="Identifier"){const firstArgValue=firstArg.value;const secondArgName=secondArg.name;if(firstArgValue!==secondArgName){context.report({node:node,loc:node.loc,message:`namespace key must match value - key: ${firstArgValue}, value: ${secondArgName}`})}}}}}}}}};module.exports.schema=[];